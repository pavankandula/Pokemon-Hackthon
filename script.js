...
async function getPokemons(){
    const data = await fetch("https://pokeapi.co/api/v2/pokemon/",{         // fetch the data from api
        method:"GET"
    })
    ...
    //console.log(dataX);
    const noOfPages = Math.ceil(dataX.length / 5);
    ...
    page.onclick = function () {
        ...
        z--;
        const pageUsers = dataX.slice((z*5),(z*5)+5);
        document.querySelector(".pokemonContent").innerHTML = "";
        Split(pageUsers);
    };
    
    }
    const firstTenUsers = dataX.slice(0, 5);
    // console.log(firstTenUsers);
    // console.log("No of Pokemons are ", dataX.length);
    Split(firstTenUsers);
    function Split(firstTenUsers){
        firstTenUsers.forEach((pokemon) => {     // getting the url of each object present in the data
...
async function getInnerData(){
    ...
    let innerData = await fetch(url,{   // fetching the innnerData present in each url 
